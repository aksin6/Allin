...
echo "║                              \__/                                  ║"
echo "║                                                                   ║"
echo "║        🛡️  GUDEL PROTECT + PANEL BUILDER  🛡️                      ║"
...
echo ""
read -p "$(echo -e ${CYAN}${BOLD}"Masukkan pilihan (1/2): "${RESET})" MODE
...
if [[ "$MODE" == "1" ]]; then
    ...
    for name in "${!CONTROLLERS[@]}"; do
        ...
        in_func == 1 && /^\s*{/ {
            ...
            print "            abort(403, \"gudel Protect - Akses ditolak\");";
            print "        }";
            in_func = 0; next;
        }
        { print; }
    ...
    exit 1
fi        echo -e "${GREEN}✅ Protect diterapkan ke: $name${RESET}"
    done
